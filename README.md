...
**③ 全量读取和 `--tail` 是两条互不相干的路径 —— 会「分裂」**

这条最关键，也是 2026-09-17 在真实环境里复盘出来的（之前只做过人为构造的实验，结论不完整）：

| 读取方式 | 扫描方向 | 坏行之后的内容 |
|---|---|---|
| `docker logs` / `logs -f`（全量） | 从文件头**顺序**解析，遇坏行即停 | **读不到** |
| `docker logs --tail N` | 从文件尾**反向**读 | **能读到** |

真实数据（同一个容器、同一时刻）：

```
docker logs --tail 1  →  2026-09-17T15:17:57Z Setting breakpad minidump AppID = 346110   ← 本次启动的
docker logs           →  最后一行是 2026-09-15T16:44:40Z CtrlCHandler: Signal=1          ← 上一次的
docker logs | wc -l   →  257 行，按日期统计：257 条全是 2026-09-15，一条 09-17 都没有
```

于是出现了最容易误导人的现象：**`--tail` 看得到本次的新日志，而 `docker logs -f`（全量路径）
看到的全是上一次的旧内容**。用 `logs -f` 排查时会坚定地认为"日志是旧的 / 服务端没重启"，
但服务端其实是好的（当时 A2S 探活正常、进程 PID 已经从 815 换成了 462）。

**所以判定日志好坏不能只看 `--tail`**（那样会漏检，本项目就漏过一次），
必须把两条路径的最后一行拿出来对比：全量读到的比 `--tail` 读到的还旧 ⇒ 中间有坏行。

顺带一个坑：`docker logs --since <时间>` 在这种状态下也会返回 **0 行**——
因为 since 过滤后依然是从头扫，照样停在坏行，且坏行之前的旧内容又被 since 过滤掉了。
别拿 `--since` 当"只看本次启动之后"的手段。

#### 现在会自动处理

`./start.sh` 加了「日志流可读性」探针：**对比全量读取与 `--tail` 读取的最后一行**，
全量读到的更旧就判定为日志损坏。检测到之后：

- 启动流程中（[3] 步）：**自动 `--force-recreate`** 重建容器拿一份干净日志
  （存档和模组都在卷里，不丢）；
- 服务端本来就在跑时：只提示、不打断游戏，让你自己挑时间执行 `./start.sh --force`。

```
--- 服务端已在运行时的提示 ---
   ⚠ 但检测到：这个容器的日志已经读不全了。
     --tail 能看到本次的新日志，可全量 docker logs / logs -f 会停在
     更早的旧内容上（json 日志中间有坏行）—— 也就是「看不到新日志」。
     服务端本身不受影响、可以正常进游戏，只是你看不到日志。
     要恢复日志： ./start.sh --force   （重建容器，存档不会丢）
```

探针刻意**不直接读** `/var/lib/docker/.../*-json.log`（属主 root，普通用户读不了），
而是拿 `docker logs` 自己当探针，因此普通用户也能用。

另外，判断"服务端起来了没有"也**不再依赖日志**（日志不可信），改为直接查容器里有没有
ARK 主进程（`pgrep -f '[S]hooterGameServer'`）。以前是 grep 日志里的 `服务端进程 PID=`，
但容器不重建时日志是累积的，上一次的 PID 行还在 —— 会导致第一轮就命中、秒报"已出现"，
哪怕本次其实还卡在 Steam 下载。
